...
class GstFakeSink(GstBaseSink):
    ...
    def __init__(self, pipeline, elem_id=0, src_video_tee=None, src_audio_tee=None):
        super().__init__(pipeline=pipeline, elem_id=elem_id,
                         src_video_tee=src_video_tee, src_audio_tee=src_audio_tee)
    ...
    def __create_fake_sink(self):
        ...
        self.__fake_sink = make_gst_element(
            "fakesink", "fakesink_%u" % self._elem_id, "fakesink_%u" % self._elem_id)
        self.get_pipeline().add(self.__fake_sink)
        self.__logger.debug("Fake sink created")

    def __create_queue(self):
        self.__logger.debug("Creating queue")
        self.__queue = make_gst_element(
            "queue", "queue-fake_%u" % self._elem_id, "queue-fake_%u" % self._elem_id)

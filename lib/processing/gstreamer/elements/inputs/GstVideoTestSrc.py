...
class GstVideoTestSrc(GstBaseSrc):
    ...
    def _create_video_src(self):
        ...
        self._video_src = make_gst_element("videotestsrc", elem_name, elem_name)
        self._video_src.set_property("pattern", "ball")
        self._pipeline.add(self._video_src)
        self.__logger.debug("Video source {} created for source {}".format(elem_name, self._source_id))

    def _create_src_caps_filter(self):
        self.__logger.debug("Creating src caps filter")
        elem_name = "src-caps-filter-%u" % self._source_id
        self._src_caps_filter = make_gst_element("capsfilter", elem_name, elem_name)
        self._src_caps_filter.set_property("caps", Gst.Caps.from_string(
            "video/x-raw, width=640, height=480, framerate=30/1, format=I420"))
        self._pipeline.add(self._src_caps_filter)
    ...
    def _create_convert(self):
        ...
        self._video_parse = make_gst_element("nvvideoconvert", elem_name, elem_name)
